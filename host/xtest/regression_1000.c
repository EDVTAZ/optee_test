...
static void xtest_tee_test_1018(ADBG_Case_t *c)
{
	...
	TEEC_SharedMemory shm;
	size_t page_size = 4096;
	uint8_t buffer[6001];	/* intentionnally not 4kB aligned and odd */
	...
	Do_ADBG_BeginSubCase(c, "Out of bounds ref (> 4kB) on alloced shm");
	...
	ret = TEEC_InvokeCommand(&session, TA_OS_TEST_CMD_PARAMS, &op, &orig);
	ADBG_EXPECT(c, TEEC_ORIGIN_COMMS, orig);
	if (ret != TEEC_ERROR_BAD_PARAMETERS && ret != TEEC_ERROR_GENERIC) {
		ADBG_EXPECT(c, TEEC_ERROR_BAD_PARAMETERS, ret);
		ADBG_EXPECT(c, TEEC_ERROR_GENERIC, ret);
	}
	...
	Do_ADBG_BeginSubCase(c, "Out of bounds by 1 byte ref on registered shm");
